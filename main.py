...
class DiseasePrediction:
    ...
    def _feature_correlation(self, data_frame=None, show_fig=False):
        # Get Feature Correlation - only on numeric feature columns, exclude prognosis
        numeric_cols = data_frame.select_dtypes(include=['int64', 'float64']).columns
        # Exclude prognosis column if it exists
        if 'prognosis' in numeric_cols:
            numeric_cols = numeric_cols.drop('prognosis')
    ...
    def select_model(self):
        if self.model_name == 'decision_tree':
            ...
            # Theory: AdaBoost is an ensemble method that combines multiple weak learners
            # (typically decision stumps - single-level decision trees) into a strong classifier.
            # 
            # How it works:
            # 1. Initialize: All training samples get equal weights
            # 2. For each iteration (n_estimators):
            #    a. Train a weak learner (decision stump) on weighted data
            #    b. Calculate error rate and learner weight (alpha)
            #    c. Update sample weights: increase for misclassified, decrease for correct
            ...
            # - learning_rate: Shrinks contribution of each learner (prevents overfitting)
            # - algorithm: SAMME.R (better, uses probability estimates) or SAMME (discrete)
            # - base_estimator: Weak learner (decision stump with max_depth=1)
            
            # Create base estimator (weak learner) - decision stump
            base_estimator = DecisionTreeClassifier(
                max_depth=self.config['model']['adaboost']['base_estimator_max_depth'],
                criterion=self.config['model']['adaboost']['base_estimator_criterion']
            )
            
            # Create AdaBoost classifier following the adaptive boosting algorithm
            # Note: scikit-learn >= 1.2 uses 'estimator', older versions use 'base_estimator'
            # Note: 'algorithm' parameter is deprecated in scikit-learn 1.6+, so we don't use it
            try:
                # Try new API first (scikit-learn >= 1.2)
                self.clf = AdaBoostClassifier(
                    estimator=base_estimator,
                    n_estimators=self.config['model']['adaboost']['n_estimators'],
                    learning_rate=self.config['model']['adaboost']['learning_rate']
                )
            except TypeError:
